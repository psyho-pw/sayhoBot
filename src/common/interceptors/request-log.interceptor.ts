  ...
  async intercept(context: ExecutionContext, next: CallHandler): Promise<any> {
    ...
    if (context.getType() === 'http') {
      ...
      try {
        ...
        this.loggerService.warn({ctx: this.intercept.name, info: req.body, message: 'failed formatting request body'});
      }
      ...
      this.loggerService.info({ctx: this.intercept.name, info: request, message: `REQUEST [${request.method}]${request.url}`});
    }
      ...
      tap((resBody) => {
        if (context.getType() === 'http') {
          ...
          try {
            ...
            this.loggerService.warn({ctx: this.intercept.name, info: resBody, message: 'failed formatting res body'});
          }
          ...
          if (responseTime >= 10_000) {
            this.loggerService.error({ctx: this.intercept.name, info: { request, response }, message: `SLOW REQUEST [${request.method}]${request.url} - ${responseTime}ms`});
          } else {
            this.loggerService.info({ctx: this.intercept.name, info: { request, response }, message: `RESPONSE [${request.method}]${request.url}`});
          }
        }
      }),
    );
