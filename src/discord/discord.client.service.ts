...
export class DiscordClientService {
  ...
  public async init() {
    ...
    try {
      this.rest = new REST({ version: '10' }).setToken(
        this.configService.discordConfig.TOKEN,
      );
      await this.discordBotClient.login(
        this.configService.discordConfig.TOKEN,
      );
      this.logger.verbose({
        ctx: this.init.name,
  ...
  public async playSong(message: Message | ChatInputCommandInteraction) {
    ...
    const nextSong: Song | null = this.formatVideo(
      video,
    ...
    const resource = await this.streamService.createAudioResourceFromUrl(
      musicQueue[0].url,
    );

    if (!message.guild)
      return channel.send(
        `Error occurred on joining voice channel\nguild is not defined`,
      );
  ...
  public setDeleteQueue(
    guildId: string,
    message: Message | InteractionResponse,
  ): void {
    if (!guildId.length)
      throw new DiscordException(
        'guildId not specified',
        'client',
        this.setDeleteQueue.name,
      );
  ...
  public getPlayer(guildId: string): AudioPlayer {
    ...
    if (!player)
      throw new DiscordException('No player found', 'client', this.getPlayer.name);
    return player;
  }
